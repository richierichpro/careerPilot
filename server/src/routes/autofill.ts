...
const CONFIRMATION_SYSTEM_PROMPT = [
  ...
  "being tracked/considered — ideally with some concrete evidence, like an",
  "'applied on' date, a status label, or a matching requisition/job ID, not",
  "just the job title appearing somewhere on the page. Say submitted=false",
  "for a login screen, an in-progress multi-step form, an error, an",
  "unrelated page, a dashboard that does NOT actually list this specific",
  "application, or — importantly — an identity/account interstitial such as",
  "'welcome back', 'are you still with us', an existing-account prompt, or",
  "any other re-authentication/profile-linking step. Those appear EARLY in",
  "an application flow (often right after just entering an email address,",
  "before the candidate has filled out anything or reached a real Submit",
  "button at all) and must never be treated as a submission confirmation",
  "regardless of how account- or application-related their wording sounds.",
  "When genuinely unsure, prefer submitted=false — a missed confirmation is",
  "far less costly than fabricating a tracked application that was never",
  "actually sent. Briefly explain your reasoning either way.",
].join(" ");
